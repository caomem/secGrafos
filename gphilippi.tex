...
Aos elementos desses conjuntos ($V_G$ e $E_G$), refere-se-os por \textit{vértices} e \textit{arestas}. Também, para um elemento $e \in E_G$, onde $e = \{u, v\}$, diz-se que $u$ e $v$ são \textit{vértices adjacentes}. Chama-se $u$ e $e$ de \textit{incidentes}, assim como $v$ e $e$. À quantidade de vértices adjacentes a $v$ da-se o nome \textit{grau} de $v$. Se duas arestas distintas $e_1$ e $e_2$ são incidentes com um vértice em comum, diz-se que $e_1$ e $e_2$ são \textit{arestas adjacentes}. 
...
Seja $G = (V_G, E_G)$ um grafo rotulado. Um \textit{passeio} $P$ \textit{em} $G$ é uma sequência dos vértices de $V_G$ finita e não nula $P = v_0,\dots, v_i, \dots, v_k$ tal que, para $i, k\in\mathbf{Z}$ e $0\le i \le |k-1|$, sempre exista $(v_i,v_{i+1}) \in E_G$.
